...
old_age = False # Смерть от старости
...
while running:
    for event in pygame.event.get(): # Обработка клавиш
        ...
        if event.type == pygame.KEYDOWN:
            ...
            elif event.key == pygame.K_s: # Включить/выключить смерть от старости
                old_age = not(old_age)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.pos[0] < map.size_window[0] and event.pos[1] < map.size_window[1]:
                ...
                if event.button == 1:  # ЛКМ - копирование
                ...
                elif event.button == 3:  # ПКМ - вставка
    ...
    for y in range(map.size[1]): # Отрисовка
        for x in range(map.size[0]):
    ...
    if not(paused): # Просчет мозгов
        ...
        for x, y, cell in entities_to_process:
            ...
            action = cell.brain(iteration, object_input, map.size) # Итерация ИИ
            ...
            if old_age: # Смерть от старости
                if random.randint(0, 100) < cell.age // 100:
                    map.map[y][x] = None
